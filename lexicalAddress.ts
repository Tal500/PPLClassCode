...
import { makeBoolExp, makeLitExp, makeNumExp, makeStrExp, makeVarDecl, makeVarRef } from './L3-ast';
import { isArray, isBoolean, isEmpty, isNumber, isNumericString, isSexpString, isString } from './L3-ast';
import { parseLitExp } from './L3-ast';
import { isError, safeF, safeFL } from './error';
...
import { SExp } from './L3-value';
import { isCompoundSExp, isEmptySExp, isSymbolSExp } from './L3-value';
...
*/
export const crossContour = (decls: VarDecl[], addresses: LexAddress[]): LexAddress[] =>
    makeBoundAddresses(decls).concat(map(makeDeeperLexicalAddress, addresses));
...
export const addLexicalAddresses = (exp: CExpLA | Error): CExpLA | Error => {
    const visitProc = (proc: ProcExpLA, addresses: LexAddress[]): ProcExpLA | Error => {
